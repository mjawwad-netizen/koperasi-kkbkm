...
import { POIN_PER_RP, STOK_WARNING, fmt, tglNow, compressImage } from "../config";
...
export default function Kasir() {
  ...
  const printNota = (trx) => {
    ...
    const items = trx.items.map(it =>
      "<tr><td>" + it.nama + "</td><td style='text-align:center'>" + it.qty + "</td><td style='text-align:right'>" + fmt(it.harga) + "</td><td style='text-align:right'>" + fmt(it.subtotal) + "</td></tr>"
    ).join("");
    w.document.write("<!DOCTYPE html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Nota</title>" +
      "<style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Courier New',monospace;padding:20px;max-width:500px;margin:0 auto;font-size:16px;line-height:1.5}.center{text-align:center}hr{border:none;border-top:2px dashed #333;margin:12px 0}table{width:100%;border-collapse:collapse}th,td{padding:6px 4px;font-size:15px}th{text-align:left;border-bottom:1px solid #333}.total-row td{font-size:18px;font-weight:700;padding-top:10px}.summary td{font-size:15px;padding:3px 4px}.footer{text-align:center;margin-top:16px;font-size:14px;color:#555}.poin{text-align:center;font-size:15px;font-weight:700;color:#16A34A;margin:8px 0}.btn-row{text-align:center;margin-top:20px}.btn-row button{padding:12px 28px;font-size:16px;cursor:pointer;border:none;border-radius:8px;margin:0 6px;font-weight:600}.btn-print{background:#2563EB;color:white}.btn-close{background:#E2E8F0;color:#333}@media print{.no-print{display:none !important}body{padding:8px;font-size:14px}th,td{font-size:13px;padding:4px 2px}.total-row td{font-size:16px}}</style></head><body>" +
      "<div class='center'><strong style='font-size:22px'>KOPERASI BAZARA</strong><br>Kabupaten Malang<br>Dari kita untuk kita</div><hr>" +
      "<div>No: " + trx.id + "<br>Tanggal: " + trx.tgl + " " + trx.waktu + "<br>Pembeli: " + (trx.pembeli?.nama || "-") + (trx.pembeli?.type === "anggota" ? " (" + trx.pembeli.id + ")" : "") + "</div><hr>" +
      "<table><tr style='font-weight:700'><td>Barang</td><td style='text-align:center'>Qty</td><td style='text-align:right'>Harga</td><td style='text-align:right'>Total</td></tr>" + items + "</table><hr>" +
      "<table><tr class='total-row'><td>TOTAL</td><td colspan='3' style='text-align:right'>Rp " + fmt(trx.total) + "</td></tr><tr class='summary'><td>Bayar</td><td colspan='3' style='text-align:right'>Rp " + fmt(trx.bayar) + "</td></tr><tr class='summary'><td>Kembalian</td><td colspan='3' style='text-align:right'>Rp " + fmt(trx.kembalian) + "</td></tr></table><hr>" +
      ...
      "<div class='btn-row no-print'><button class='btn-print' onclick='window.print()'>Cetak</button> <button class='btn-close' onclick='window.close()'>Tutup</button></div>" +
      "</body></html>");
    w.document.close();
  };
  ...
  return (<>
    ...
    {sub === "kasir" && (<>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginBottom: 12 }}>
        {filteredBarang.map(b => {
          ...
          return (
            <div key={b.id} style={{ ...S.card, marginBottom: 0, padding: 0, overflow: "hidden", cursor: b.stok > 0 ? "pointer" : "default", opacity: b.stok <= 0 ? 0.5 : 1 }} onClick={() => addToCart(b)}>
              {b.foto ? (
                <img src={b.foto} alt={b.nama} style={{ width: "100%", height: 80, objectFit: "cover" }} />
              ) : (
                <div style={{ width: "100%", height: 50, background: "#F1F5F9", display: "flex", alignItems: "center", justifyContent: "center", color: "#CBD5E1", fontSize: 20 }}>📦</div>
              )}
              <div style={{ padding: "6px 8px" }}>
                <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 2 }}>{b.nama}</div>
                <div style={{ fontSize: 14, color: "#2563EB", fontWeight: 700 }}>Rp {fmt(b.hargaJual)}</div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 2 }}>
                  <span style={{ fontSize: 10, color: b.stok <= STOK_WARNING ? "#DC2626" : "#94A3B8" }}>Stok: {b.stok}</span>
                  {inCart && <span style={{ background: "#2563EB", color: "white", fontSize: 10, padding: "1px 7px", borderRadius: 10, fontWeight: 600 }}>{inCart.qty}x</span>}
                </div>
              </div>
            </div>
          );
        ...
        {cart.map(c => (
          <div key={c.id} style={{ ...S.card, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: "#64748B" }}>Rp {fmt(c.hargaJual)} x {c.qty} = <span style={{ color: "#2563EB", fontWeight: 600 }}>Rp {fmt(c.hargaJual * c.qty)}</span></div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <button style={S.qtyBtn} onClick={() => updateQty(c.id, 1)}>+</button>
              <button style={{ ...S.qtyBtn, background: "#DC2626", marginLeft: 4, fontSize: 12 }} onClick={() => setCart(cart.filter(x => x.id !== c.id))}>x</button>
            </div>
          </div>
        ))}
    ...
    {sub === "stok" && (<>
      ...
      {barang.map(b => (<div key={b.id} style={{ ...S.card, display: "flex", alignItems: "center", gap: 10 }}>
        {b.foto ? <img src={b.foto} alt={b.nama} style={{ width: 48, height: 48, borderRadius: 8, objectFit: "cover" }} /> : <div style={{ width: 48, height: 48, borderRadius: 8, background: "#F1F5F9", display: "flex", alignItems: "center", justifyContent: "center", color: "#CBD5E1", fontSize: 18, flexShrink: 0 }}>📦</div>}
        <div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 600 }}>{b.nama}</div><div style={{ fontSize: 11, color: "#64748B" }}>{b.kode} · {b.kategori}</div><div style={{ fontSize: 12, marginTop: 2 }}><span style={{ color: "#64748B" }}>Beli: {fmt(b.hargaBeli)}</span> · <span style={{ color: "#2563EB", fontWeight: 500 }}>Jual: {fmt(b.hargaJual)}</span></div></div>
        <div style={{ textAlign: "right" }}><div style={{ fontSize: 16, fontWeight: 700, color: b.stok <= STOK_WARNING ? "#DC2626" : "#1E293B" }}>{b.stok}</div><div style={{ display: "flex", gap: 4, marginTop: 4 }}><button style={S.btnSm("#64748B")} onClick={() => setEditB(b)}>Edit</button><button style={S.btnSm("#F59E0B")} onClick={() => setShowRestok(b)}>+Stok</button></div></div>
      </div>))}
    ...
    {showBarangForm && (() => {
      const F = () => {
        ...
        const handleFoto = async (e) => { const file = e.target.files?.[0]; if (!file) return; setUploading(true); setF({ ...f, foto: await compressImage(file, 300, 0.5) }); setUploading(false); };
        return (<div style={S.modal} onClick={() => setShowBarangForm(false)}><div style={S.modalContent} onClick={e => e.stopPropagation()}>
          <div style={S.modalHeader}><span style={{ fontSize: 16, fontWeight: 600 }}>Tambah barang</span><button onClick={() => setShowBarangForm(false)} style={S.closeBtn}>x</button></div>
    ...
    {editB && (() => {
      const F = () => {
        ...
        return (<div style={S.modal} onClick={() => setEditB(null)}><div style={S.modalContent} onClick={e => e.stopPropagation()}>
          <div style={S.modalHeader}><span style={{ fontSize: 16, fontWeight: 600 }}>Edit barang</span><button onClick={() => setEditB(null)} style={S.closeBtn}>x</button></div>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
    ...
    {showRestok && (() => {
      const F = () => {
        ...
        return (<div style={S.modal} onClick={() => setShowRestok(null)}><div style={{ ...S.modalContent, paddingBottom: 24 }} onClick={e => e.stopPropagation()}>
          <div style={S.modalHeader}><span style={{ fontSize: 16, fontWeight: 600 }}>Restok: {showRestok.nama}</span><button onClick={() => setShowRestok(null)} style={S.closeBtn}>x</button></div>
          <div style={{ fontSize: 13, color: "#64748B", marginBottom: 12 }}>Stok saat ini: <strong>{showRestok.stok}</strong></div>
          <input style={{ ...S.input, marginBottom: 8 }} placeholder="Jumlah tambahan" type="number" value={qty} onChange={e => setQty(e.target.value)} />
          <input style={{ ...S.input, marginBottom: 10 }} placeholder="Total biaya beli (Rp)" type="number" value={biaya} onChange={e => setBiaya(e.target.value)} />
          <button style={S.btn()} onClick={() => { if (parseInt(qty) > 0) { restokBarang(showRestok.id, parseInt(qty), parseInt(biaya) || 0); setShowRestok(null); } }}>Tambah stok</button>
        </div></div>);
      }; return <F />;
    })()}
    ...
    {showStruk && (<div style={S.modal} onClick={() => setShowStruk(null)}><div style={{ ...S.modalContent, textAlign: "center" }} onClick={e => e.stopPropagation()}>
      ...
      <div style={{ textAlign: "left", borderTop: "1px dashed #CBD5E1", paddingTop: 12 }}>
        {showStruk.items.map((it, i) => (<div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: 13, padding: "4px 0" }}><span>{it.nama} x{it.qty}</span><span style={{ fontWeight: 500 }}>Rp {fmt(it.subtotal)}</span></div>))}
    ...
    {/* RIWAYAT */}
    {showHistori && (<div style={S.modal} onClick={() => setShowHistori(false)}><div style={S.modalContent} onClick={e => e.stopPropagation()}>
      <div style={S.modalHeader}><span style={{ fontSize: 16, fontWeight: 600 }}>Riwayat transaksi</span><button onClick={() => setShowHistori(false)} style={S.closeBtn}>x</button></div>
      {transaksi.length === 0 ? <div style={{ textAlign: "center", padding: 30, color: "#94A3B8", fontSize: 13 }}>Belum ada</div> : transaksi.slice(0, 20).map(t => (
        <div key={t.id} style={{ ...S.card, cursor: "pointer" }} onClick={() => { setShowHistori(false); setShowStruk(t); }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div><div style={{ fontSize: 13, fontWeight: 600 }}>{t.tgl} - {t.waktu}</div><div style={{ fontSize: 11, color: "#64748B" }}>{t.pembeli?.nama || "-"} - {t.items.length} item</div></div>
