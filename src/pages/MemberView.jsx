...
function BayarModal({ userId, onClose, addPembayaran }) {
  ...
  if (jenis === "Simpanan wajib") {
    ...
    labelBayar = jumlahBulan + " bulan x Rp " + fmt(SIMPANAN_WAJIB) + " = Rp " + fmt(jumlahBayar);
  } else {
  ...
  const kirim = () => {
    ...
    addPembayaran({ id: "PB-" + Date.now(), anggotaId: userId, tgl: tglNow(), waktu: waktuNow(), jenis: jenis, jumlah: jumlahBayar, bukti: buktiText, buktiImg: buktiImg || "", status: "menunggu", keterangan: keteranganFinal });
    setDone(true);
  };
